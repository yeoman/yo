...
function init() {
  ...
  let env;
  if (firstCmd.opts.env) {
    env = Env.createEnvWithVersion(firstCmd.opts.env);
    const installedVersion = Env.repository.getPackageVersion('yeoman-environment');
    console.log(`Running environment ${installedVersion}`);
  } else {
    env = Env.createEnv();
  }
