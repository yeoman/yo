...
function init() {
  ...
  env.lookup(function () {
    ...
    if (!cmd) {
      if (opts.help) {
        var generators = Object.keys(env.getGeneratorsMeta()).reduce(function (acc, gen) {
          var parts = gen.split(':');
          var genName = parts.shift();
          ...
          if (parts[0] !== 'app') {
            acc[genName].push(parts.join(':'));
          }

          return acc;
        }, {});

        var genList = Object.keys(generators).map(function (gen) {
          return '  ' + gen + '\n' + generators[gen].map(function (sub) { return '    '+sub; }).join('\n');
        }).join('\n');
